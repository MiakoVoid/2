...
import com.example.greenfinance.data.repository.CategoryRepositoryImpl;
import com.example.greenfinance.presentation.calendar.adapter.CategoryAdapter;
...
public class ManualBillingActivity extends AppCompatActivity implements CategoryAdapter.OnCategoryClickListener {
    private static final String TAG = "ManualBillingActivity";
    ...
    private EditText etOrderNumber; // 添加订单编号输入框
    ...
    private void initViews() {
        ...
        etOrderNumber = findViewById(R.id.et_order_number); // 初始化订单编号输入框
    ...
    private void loadCategories() {
        // 从后端获取分类数据
        categoryRepository.getCategories(isExpense ? 1 : 2).observe(this, new Observer<BaseResponse<List<Category>>>() {
            @Override
            public void onChanged(BaseResponse<List<Category>> response) {
                if (response != null && response.getData() != null) {
                    List<CategoryAdapter.CategoryItem> categoryItems = CategoryAdapter.createCategoryItems(response.getData());
                    categoryAdapter.updateCategories(categoryItems);
                } else {
                    // 加载失败时使用模拟数据
                    Log.e(TAG, "加载分类失败，使用模拟数据");
                    loadMockCategories();
                }
            }
        });
    ...
    private void saveBill() {
        ...
        String orderNumber = etOrderNumber.getText().toString().trim(); // 获取订单编号
        ...
        try {
            ...
            String subCategoryName = getSubCategoryNameById(selectedSubCategoryId);
            bill.setCategoryName(categoryName);
            bill.setSubCategoryName(subCategoryName);
            
            bill.setMerchant(merchant.isEmpty() ? "未填写" : merchant);
            bill.setRemark(remark);
            bill.setOrderNumber(orderNumber); // 设置订单编号
            ...
            // 保存账单到仓库
            // BillRepository.getInstance().saveBill(bill);
    ...
    private String getSubCategoryNameById(Long subCategoryId) {
        // 模拟获取子分类名称
